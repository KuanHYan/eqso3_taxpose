...
class AttentionHead(nn.Module):
    def __init__(
        ...
        self.project_q = nn.Conv1d(emb_dims, emb_dims, kernel_size=1, bias=False)
    ...
    def _simple_attn(self, query, key):
        ...
        key = self.project_k(key)
        d_k = query.size(1)
        scores = torch.matmul(
            query.transpose(2, 1).contiguous(), key
        ) / math.sqrt(d_k)
    ...
    def forward(self, *input, scores):
        ...
        ## TODO: 测试使用action_pts or anchor_pts
        rel_vec = anchor_points_i - anchor_points_j  # B, 3, N, N
        del anchor_points_i, anchor_points_j
        scores_for_bias = self._simple_attn(action_embedding, action_embedding)
        residual_flow = torch.einsum("bcnm,bmn->bcn", rel_vec, scores_for_bias)
        del rel_vec
...
class WeightHead(nn.Module):
    def __init__(self, emb_dims=512, pred_weight=True, output_num=1024, norm=nn.BatchNorm1d):
        ...
        if pred_weight:
            ...
            self.proj_flow_weight = None
        self.emb_dims = emb_dims
        self.project_emb = weight_norm(nn.Linear(output_num, output_num, bias=False))
        self.pred_weight = pred_weight
    ...
    def forward(self, *input, scores=None, return_flow_component=False):
        ...
        return:
          ...
          scores: B,N,N
        """
        # TODO: 理论上act_emb和anch_emb都需要Normalization. 
        # 但现在action_embedding是TF输出，并未进行Normalization
        # 但锚点embedding是DGCNN输出，已经进行Normalization。
        # 考虑求相似度时会自动归一化，所以此处不需要归一化
        action_embedding = input[0]
        ...
        target_emb = self.project_emb(anchor_embedding)
        emb_similarity = F.cosine_similarity(target_emb, action_embedding, dim=1)
        anchor_points_center = anchor_points.mean(dim=2, keepdim=True)
        corr_points = self.project_emb(anchor_points - anchor_points_center)
        corr_points += anchor_points_center
        corr_flow = corr_points - action_points
